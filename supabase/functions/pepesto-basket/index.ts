...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (mode === "checkout") {
      ...
      await client.rpc("record_usage_event", { p_category: "pepesto_checkout", p_cost: 0.32, p_metadata: { retailer: retailerDomain } }).catch(() => {});
      return json(req, {
    ...
    await client.from("shopping_lists").update({ pepesto_last_compared_at: new Date().toISOString() })
      .eq("id", list.id);
    await client.rpc("record_usage_event", { p_category: "pepesto_compare", p_cost: 0.04, p_metadata: { retailer: retailerDomain } }).catch(() => {});
